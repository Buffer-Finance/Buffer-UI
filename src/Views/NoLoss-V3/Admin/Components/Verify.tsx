...
export const Verify = () => {
  ...
  const filteredTournaments = useMemo(() => {
    ...
    return allTournaments.filter(
      (tournament) => tournament.tournamentMeta.isVerified === false
    );
  }, [allTournaments]);

  async function handleVerify(tournamentId: string) {
    try {
    ...
    <div className="flex flex-wrap gap-4 text-f14 p-4">
      {filteredTournaments.map((tournament) => (
        <AdminTournamentCard
          ...
          button={
            <BlueBtn
              onClick={() => handleVerify(tournament.id)}
