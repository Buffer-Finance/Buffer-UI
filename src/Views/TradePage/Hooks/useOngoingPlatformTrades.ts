  ...
  const { data, error } = useSWR<tradesApiResponseType>(
      ...
      activePage,
    {
      fetcher: async () => {
        ...
        const res = await axios.get(`${upDOwnV3BaseUrl}trades/all_active/`, {
          params: {
      ...
      activePage,
    {
      fetcher: async () => {
        ...
        if (
          ...
          return { page_data: [], total_pages: 1 };
        const res = await axios.get(`${upDOwnV3BaseUrl}trades/all_history/`, {
          params: {
            user_address: address,
            environment: activeChain.id,
            limit: TRADE_IN_A_PAGE_TRADES_TABLES,
      ...
      activePage,
    {
      fetcher: async () => {
        ...
        if (
          ...
          return { page_data: [], total_pages: 1 };
        const res = await axios.get(`${upDOwnV3BaseUrl}trades/all_cancelled/`, {
          params: {
            user_address: address,
            environment: activeChain.id,
            limit: TRADE_IN_A_PAGE_TRADES_TABLES,
