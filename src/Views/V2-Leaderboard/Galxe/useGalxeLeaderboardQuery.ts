...
import { useGalxeWeekOffset } from '../Hooks/useGalxeOffset';
...
export const useGalxeLeaderboardQuery = () => {
  ...
  const { offset } = useGalxeWeekOffset();
  const { activeChain } = useActiveChain();
  const graphUrl = `https://subgraph.satsuma-prod.com/e66b06ce96d2/bufferfinance/arbitrum-mainnet/version/v2.7.2-galex-leaderboards-2/api`;
    ...
    () =>
      poolNames.filter(
        (pool) =>
          ...
          !pool.toLowerCase().includes('usdc.e')
      ),
    [poolNames]
  );
