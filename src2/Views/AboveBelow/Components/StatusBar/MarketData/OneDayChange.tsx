...
import { usePriceChangeAboveBelow } from '@Views/AboveBelow/Hooks/usePriceCHangeAboveBelow';
...
export const OneDayChange: React.FC<{
  ...
  const assetPrices = usePriceChangeAboveBelow();
  if (activeMarket === undefined)
    return <Skeleton className="w-[50px] !h-5 lc " />;
  const oneDayChange =
    assetPrices?.[activeMarket.token0 + activeMarket.token1]?.change;
